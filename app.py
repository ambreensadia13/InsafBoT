Yes. The previous response accidentally included non-Python text after the script. Below is the **proper `app.py`**.

**Copy only the code inside this block into GitHub as `app.py`.** The first line is `import html`, and there are no Markdown backticks inside the file.
...
4. Base the answer primarily on the supplied legal documents.
5. If the documents do not provide enough information, clearly say so.
6. Explain legal concepts clearly and practically.
7. Mention the relevant law, section, article, ordinance, or regulation when it is supported by the context.
8. Never guess an exact legal provision.
9. For serious or urgent matters, recommend consulting a qualified Pakistani lawyer or relevant authority.
10. Do not claim to create an attorney-client relationship.
11. Do not unnecessarily repeat the user's question.
12. Answer in the selected language.
...
st.markdown(
""" <div class="subtitle">
AI-powered legal information assistant for Pakistan </div>
""",
unsafe_allow_html=True,
)
...
Use a clear and practical structure where appropriate.

If the context does not contain enough information,
do not invent an answer. Clearly state that the available
legal documents do not provide enough information.
"""
...
st.markdown(
""" <div class="info-card"> <strong>Important:</strong>
...
licensed legal professional or the relevant authority. </div>
""",
unsafe_allow_html=True,
)
...
st.markdown(
""" <div class="footer">
INSAFBOT • Pakistan Legal Information Assistant </div>
""",
unsafe_allow_html=True,
)
"""

Paste the code **starting from `import html` through the final `"""`-containing CSS/string sections exactly as shown inside the writing block**. Do not paste the writing-block markers themselves.
