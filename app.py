...
Aapke uploaded code mein problematic line thi:

```python
     examples = [
```

yani `examples` ke start mein **extra space** tha. Isi wajah se `IndentationError` aaya.

Correct version mein:

```python
    examples = [
```

hai, kyunki ye `with st.sidebar:` ke andar hai.
...
hai — **title ke aage ⚖️ emoji nahi hai**.

`page_icon="⚖️"` ko maine rehne diya hai, isliye browser/Streamlit tab mein legal icon aa sakta hai, lekin **main INSAFBOT title ke saamne nahi**.
